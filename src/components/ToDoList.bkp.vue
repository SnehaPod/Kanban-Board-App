      ...
      <div class="row">
        ...
        <div class="col-lg-3 col-md-6 col-sm-12">
          ...
          <div v-for="todo in todosTasks" :key="todo.id">
            <draggable v-model="todosTasks" animation: 200 handle=".task-handle1" :id="`Tasks-${todo.id}`" group="tasks" @start="drag=true" @end="drag=false">
                  <b-card
                    text-variant="dark"
        ...
        <div class="col-lg-3 col-md-6 col-sm-12">
          ...
          <!-- List of tasks -->
            <div v-for="todo in todosCompleted" :key="todo.id">
              <draggable v-model="todosCompleted" animation: 200 handle=".task-handle2"  :id="`Completed-${todo.id}`" group="tasks" @start="drag=true" @end="drag=false">
                    <b-card
                      text-variant="dark"
                      class="text-left todo-card-title px-0 mb-2 cursor-pointer"
                      v-bind:style="{ 'background-color': todo.bgColor }"
        ...
        <div class="col-lg-3 col-md-6 col-sm-12">
          ...
          <div v-for="todo in todosDeferred" :key="todo.id">
            <draggable v-model="todosDeferred" handle=".task-handle3" :id="`Deferred-${todo.id}`" group="tasks" @start="drag=true" @end="drag=false">
                  <b-card
                    text-variant="dark"
                    class="text-left todo-card-title px-0 mb-2 cursor-pointer"
                    v-bind:style="{ 'background-color': todo.bgColor }"
        ...
        <div class="col-lg-3 col-md-6 col-sm-12">
          ...
          <div v-for="todo in todosRejected" :key="todo.id">
            <draggable v-model="todosRejected" handle=".task-handle4" :id="`Rejected-${todo.id}`" group="tasks" @start="drag=true" @end="drag=false">
                  <b-card
                    text-variant="dark"
                    class="text-left todo-card-title px-0 mb-2 cursor-pointer"
                    v-bind:style="{ 'background-color': todo.bgColor }"
...
export default {
  ...
  methods: {
    ...
    moveCards: function (event) {
      this.$drag = false;
      console.log('event.from.id', event.from.id);
      console.log('event.to.id', event.to.id);

      // var status = event.to.id.split("-")[0];
      // var id = event.from.id.split("-")[1];

      // this.todosArray = localStorage.getItem("todosArray")
      //   ? JSON.parse(localStorage.getItem("todosArray"))
      //   : [];
      // console.log((event.from.id.split("-")[0] !== event.to.id.split("-")[0]))
      // if (this.todosArray.length && (event.from.id.split("-")[0] !== event.to.id.split("-")[0])) {
      //   this.todosArray.forEach(function (el, index) {
      //     if (this[index].id == id) {
